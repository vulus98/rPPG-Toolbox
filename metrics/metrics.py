...
def calculate_metrics_SR(predictions, labels,config,signal_removal=False,video_mse=None,max_pixel_val=255):
    ...
    video_mse=video_mse.cpu().detach().numpy()
    ...
    for metric in config.TEST.METRICS:
        if metric == "MAE":
            ...
            print('MSNR: {0}'.format(torch.nanmean(snr_all)))
        else:
