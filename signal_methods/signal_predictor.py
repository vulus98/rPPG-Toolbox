...
def signal_predict(config, data_input, labels_input,method_name,diff_flag):
    ...
    labels_input=labels_input.cpu().numpy()
    #sbar = tqdm(data_loader["signal"], ncols=80)
    for i in range(data_input.shape[4]):
        #batch_size = test_batch[0].shape[0]
        #for idx in range(batch_size):
            #data_input, labels_input = test_batch[0][idx].cpu().numpy(), test_batch[1][idx].cpu().numpy()
        if method_name == "pos":
            BVP = POS_WANG(data_input[:,:,:,:,i], config.TEST.DATA.FS)
        elif method_name == "chrome":
            BVP = CHROME_DEHAAN(data_input[:,:,:,:,i], config.TEST.DATA.FS)
        elif method_name == "ica":
            BVP = ICA_POH(data_input[:,:,:,:,i], config.TEST.DATA.FS)
        elif method_name == "green":
            BVP = GREEN(data_input[:,:,:,:,i])
        elif method_name == "LGI":
            BVP = LGI(data_input[:,:,:,:,i])
        elif (method_name == "PBV"):
            BVP = PBV(data_input[:,:,:,:,i])
        else:
        ...
        if config.INFERENCE.EVALUATION_METHOD == "FFT":
            gt_fft_hr, pre_fft_hr = calculate_metric_per_video(BVP, labels_input[:,i], diff_flag=diff_flag,
                                                                fs=config.TEST.DATA.FS)
            if(pre_fft_hr>=60 and pre_fft_hr<=120):
